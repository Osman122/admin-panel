...
export default function ResturantCard (props) {
  ...
  const showMovieDetails = (id) => {
    ...
    const {theme} = useContext(Theme)
     ...
     return(  <div class={`col-12 col-sm-6 col-lg-5 pt-3 px-lg-4 `}>
      <div key={resturant.id} className={`card card-light  ${theme==="dark" ? "bg-dark text-light border-light ":""}`}>
<img onClick={() => showMovieDetails(resturant.id)} src={resturant.image} className="card-img img-fluid" alt="..." 
      ...
      <button  className ="my-3 mx-4 btn btn-primary  " onClick={() => showMovieDetails(resturant.id)}> {t("Show details")}</button>
      </div>
      </div>
